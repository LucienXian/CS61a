...
def balanced(m):
    ...
    def balance(m):
        ...
        result = []
        for s in sides(m):
            side_balance=balance(end(s))
            result+=[[side_balance[0], side_balance[1], side_balance[1]*length(s)]]
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pw):
        nonlocal bal
        if pw != password and len(try_password) < 3:
            try_password.append(pw)
            return 'Incorrect password'
        if len(try_password) == 3:
            return "Your account is locked. Attempts: "+str(try_password)
        if amount > bal:
