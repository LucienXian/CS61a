...
def reverse_other(t):
    ...
    """
    def reverse_helper(tr, need_reverse):
        if t.is_leaf():
            return
        new_branch_root = [child.root for child in tr.branches][::-1]
        for i in range(len(tr.branches)):
            reverse_helper(tr.branches[i], not need_reverse)
            if need_reverse:
                tr.branches[i].root = new_branch_root[i]
...
def has_cycle_constant(link):
    ...
    while fast is not Link.empty:
        if fast.rest is Link.empty:
            return False
        elif slow == fast or slow == fast.rest:
            return True
        else:
